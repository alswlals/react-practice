...
1. webpack은 작게 분리된 많은 애플리케이션 모듈(js, css, scss/sass, images, font)들을 '의존성 분석'을 하여 하나의 js로 묶음
2. 최종 결과물인 하나의 js 파일을 번들(bundle)이라 하고 그 묶는 작업을 번들링(bundling)이라고 한다.
...
4. 빌드 작업(js 모듈)
    ...
    [4] 난독화/압축(uglify) 작업
    [5] 번들링(bundling) 작업
5. js 모듈뿐만 아니라 다양한 assets(images, css, sass/scss, font)들도 모듈로 취급한다.
6. 설치
